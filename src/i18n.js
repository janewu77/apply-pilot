...
const TRANSLATIONS = {
  en: {
    ...
    'options.smartImport.desc':         'Smart import sends your document to the AI service configured in AI Settings, even when automatic matching is off. Use a cloud API key or Ollama (TXT/Markdown only). Review extracted details before applying.',
    ...
    'options.llm.setupForbidden': '❌ Ollama rejected this extension (403). Follow the expanded setup instructions below, restart Ollama, then test again.',
    ...
    'options.llm.ollamaLoaded': '{count} models found on your Ollama server. Your selection is saved automatically.',
    ...
    'options.llm.ollamaModelHint': 'Select a model from your Ollama server. To keep processing on your device, choose a local model.',
    'options.llm.ollamaHint': 'Start Ollama, select a model, then test the connection. First use may take longer while the model loads. Ollama import currently supports TXT and Markdown. PDF support is not yet implemented.',
  ...
  zh: {
    ...
    'options.smartImport.desc':         '智能导入会将文档发送至「AI 设置」中配置的服务，即使自动匹配已关闭。可使用云端 API Key 或 Ollama（仅支持 TXT/Markdown）。应用前请核对提取结果。',
    ...
    'options.llm.setupForbidden': '❌ Ollama 拒绝了此扩展的请求（403）。请按下方已展开的步骤配置，重启 Ollama 后再测试。',
    'options.llm.ollamaRefresh': '↻ 刷新模型',
    'options.llm.ollamaSelectModel': '请选择模型',
    'options.llm.ollamaLoading': '正在读取可用模型…',
    'options.llm.ollamaLoaded': '在你的 Ollama 服务中找到 {count} 个模型，所选模型会自动保存。',
    ...
    'options.llm.ollamaModelHint': '从你的 Ollama 服务中选择模型。如需在本机处理数据，请选择本地模型。',
    'options.llm.ollamaHint': '启动 Ollama，选择模型后测试连接。首次加载模型可能较慢。Ollama 智能导入目前支持 TXT 和 Markdown，PDF 支持尚未实现。',
